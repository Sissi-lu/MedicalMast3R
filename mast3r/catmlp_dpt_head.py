...
def postprocess_albedo(out, depth_mode, conf_mode, desc_dim=None, desc_mode='norm', two_confs=False, desc_conf_mode=None):
    fmap = out.permute(0, 2, 3, 1)  # B,H,W,D
    res = dict(pts3d=reg_dense_depth(fmap[..., 0:3], mode=depth_mode))
    res['conf'] = reg_dense_conf(fmap[..., 3], mode=conf_mode)
    ...
    if two_confs:
        res['desc_conf'] = reg_dense_conf(fmap[..., start + desc_dim], mode=desc_conf_mode)
        new_start = start + desc_dim + 1
    else:
        res['desc_conf'] = res['conf'].clone()
        new_start = start + desc_dim

    res['albedo'] = reg_desc(fmap[..., new_start:new_start + 3], mode=desc_mode)
    res['albedo_conf'] = reg_dense_conf(fmap[..., new_start+3], mode=desc_conf_mode)
    res['specular'] = reg_desc(fmap[..., new_start+4:new_start+7], mode=desc_mode)
    res['specular_conf'] = reg_dense_conf(fmap[..., new_start+7], mode=desc_conf_mode)
...
##--------------------------------------adding albedo ----------------------------------------##
class Cat_MLP_LocalFeatures_DPT_Pts3d_DPT_albedo(PixelwiseTaskWithDPT):
    """ Mixture between MLP and DPT head that outputs 3d points and local features (with MLP).
    The input for both heads is a concatenation of Encoder and Decoder outputs
    """
    ...
    def __init__(self, net, has_conf=False, local_feat_dim=16, hidden_dim_factor=4., hooks_idx=None, dim_tokens=None,
        ...
        self.head_local_features = Mlp(in_features=idim,
                                       hidden_features=int(hidden_dim_factor * idim),
                                       out_features=(self.local_feat_dim + self.two_confs) * self.patch_size**2)
        self.dpt_albedo = copy.deepcopy(self.dpt)
        self.dpt_specular = copy.deepcopy(self.dpt)

        # self.head_illumination = Mlp(in_features=idim,
        #                              hidden_features=int(hidden_dim_factor * idim),
        #                              out_features=1)

        print(self.dpt_albedo)

    def forward(self, decout, img_shape):
        # pass through the heads
        pts3d = self.dpt(decout, image_size=(img_shape[0], img_shape[1]))
        albedo = self.dpt_albedo(decout, image_size=(img_shape[0], img_shape[1]))
        specular = self.dpt_specular(decout, image_size=(img_shape[0], img_shape[1]))
        ...
        # post process 3D pts, descriptors and confidences
        out = torch.cat([pts3d, local_features, albedo, specular], dim=1)
        # if self.postprocess:
        out = postprocess_albedo(out,
                               depth_mode=self.depth_mode,
                               conf_mode=self.conf_mode,
                               desc_dim=self.local_feat_dim,
                               desc_mode=self.desc_mode,
...
def mast3r_head_factory(head_type, output_mode, net, has_conf=False):
    ...
    if head_type == 'catmlp+dpt' and output_mode.startswith('pts3d+desc'):
    ...
    elif head_type == 'catmlp+dpt+albedo':
        local_feat_dim = int(output_mode[10:])
        assert net.dec_depth > 9
        l2 = net.dec_depth
        feature_dim = 256
        ...
        ed = net.enc_embed_dim
        dd = net.dec_embed_dim
        return Cat_MLP_LocalFeatures_DPT_Pts3d_DPT_albedo(net, local_feat_dim=local_feat_dim, has_conf=has_conf,
                                               num_channels=out_nchan + has_conf,
                                               feature_dim=feature_dim,
                                               last_dim=last_dim,
                                               hooks_idx=[0, l2 * 2 // 4, l2 * 3 // 4, l2],
