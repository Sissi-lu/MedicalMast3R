...
def valid_mean(arr, mask, axis=None, keepdims=np._NoValue):
    ...
    with np.errstate(divide='ignore', invalid='ignore'):
        ...
        # Ensure valid_mean is a NumPy array
        valid_mean = np.asarray(valid_mean)
        # Replace NaN, +inf, -inf with 0
        valid_mean = np.where(np.isnan(valid_mean) | np.isinf(valid_mean), 0, valid_mean)
...
def m_rel_ae(gt, pred, mask=None, output_scaling_factor=1.0):
    ...
    # print("abs:", ae)
    # print("divide: ", ae/gt )
    # print("mask: %s, diff: %s, abs: %s, divide:%s"%(mask.dtype, e.dtype, ae.dtype, (ae/gt).dtype))
    # numpy 2.0.1 has conflict with this line
    # with np.errstate(divide='ignore', invalid='ignore'):
    #     rel_ae = np.nan_to_num(ae / gt, nan=0, posinf=0, neginf=0, copy=True)

    rel_ae = np.where(np.isnan(ae/gt) | np.isinf(ae/gt), 0, ae/gt)
