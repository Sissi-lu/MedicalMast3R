...
def parse_args():
    ...
    parser.add_argument('--input-dir', type=str, default='/data/luxiaoxi/dataset/medical_depth/endonerf')
    parser.add_argument('--input-data', type=str, help='cutting_tissues_twice or pulling_soft_tissues', default='cutting_tissues_twice')
    parser.add_argument('--output-dir', type=str, default='/data/luxiaoxi/dataset/medical_depth_output/endoscope_mast3r_zeroshot')
...
def load_data_path(img_path):
    ...
    # plt.close()
    return left_img, l_depth, right_img
...
def draw_picture(save_folder, pred_left, depth_left, img_left, img_right):
...
def endoscope_evaluation(args):
    ...
    img_list = [os.path.join(args.input_dir, args.input_data, 'left', img) for img in img_list if img.endswith('.png')]
    ...
    for img_path in metric_logger.log_every(img_list, print_freq=1, header=header):
        ...
        left_img, left_depth, right_img = load_data_path(img_path)
